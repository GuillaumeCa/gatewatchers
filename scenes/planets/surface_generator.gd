...
@export var update = false:
	set(val):
		print("update")
		call_deferred("generate_surface")
...
@export var depth = 3
@export var terrain_scale = 30

@export var debug_range = Vector2():
	set(v):
		debug_range = v
		call_deferred("generate_surface")
...
func octasphere(ndivisions: int, radius: float, width=0, height=0, depth=0) -> Array:
	...
	for i in range(n):
		...
		#if theta < debug_range.x or theta > debug_range.y: continue
	...
	for i in range(num_verts):
		#if i < max_idx:
		var v = verts[i]
		var noise_at = noise.get_noise_3dv(v)
		verts[i] *= radius + noise_at * terrain_scale
